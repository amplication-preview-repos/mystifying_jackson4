/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProductionHouseWhereInput } from "./ProductionHouseWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ProductionHouseOrderByInput } from "./ProductionHouseOrderByInput";

@ArgsType()
class ProductionHouseFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ProductionHouseWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ProductionHouseWhereInput, { nullable: true })
  @Type(() => ProductionHouseWhereInput)
  where?: ProductionHouseWhereInput;

  @ApiProperty({
    required: false,
    type: [ProductionHouseOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ProductionHouseOrderByInput], { nullable: true })
  @Type(() => ProductionHouseOrderByInput)
  orderBy?: Array<ProductionHouseOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ProductionHouseFindManyArgs as ProductionHouseFindManyArgs };
